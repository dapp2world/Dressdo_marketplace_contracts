...
// npx hardhat run --network besutestnet scripts/scripts2/deploy_all.js
async function main(network) {
  ...
  //////////
  const Artion = await ethers.getContractFactory('DressDio');
  ...
  // Modified based on the request for NFT Collections

  // IP NFT
  const artion1 = await Artion.deploy(TREASURY_ADDRESS, "0");
  await artion1.deployed();
  console.log("FantomArtion - IP NFT - deployed at", artion1.address);

  // Personal NFT
  const artion2 = await Artion.deploy(TREASURY_ADDRESS, "0");
  await artion2.deployed();
  console.log("FantomArtion - Personal NFT - deployed at", artion2.address);

  // Merchandise NFT with 1 DP Token fee
  const artion3 = await Artion.deploy(TREASURY_ADDRESS, "1000000000000000000");
  await artion3.deployed();
  console.log("FantomArtion - Merchandise NFT - deployed at", artion3.address);
  ...
  await addressRegistry.addArtion(artion.address);
  await addressRegistry.addArtion(artion1.address);
  await addressRegistry.addArtion(artion2.address);
  await addressRegistry.addArtion(artion3.address);

  await artion1.updateAddressRegistry(FANTOM_ADDRESS_REGISTRY);
  await artion2.updateAddressRegistry(FANTOM_ADDRESS_REGISTRY);
  await artion3.updateAddressRegistry(FANTOM_ADDRESS_REGISTRY);
