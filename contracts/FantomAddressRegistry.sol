...
contract FantomAddressRegistry is Ownable {
    ...
    /**
     ...
     @dev Only admin
     */
    function updateArtion(uint16 _index, address _artion) external onlyOwner {
        require(
            IERC165(_artion).supportsInterface(INTERFACE_ID_ERC721),
            "Not ERC721"
        );
        artion[_index] = _artion;
    }
    ...
    /**
     ...
     @dev Only admin
     */
    function addArtion(address _artion) external onlyOwner {
        require(
            IERC165(_artion).supportsInterface(INTERFACE_ID_ERC721),
            "Not ERC721"
        );
    ...
    function artionExists(address _artion) external view returns (bool) {
        require(
            IERC165(_artion).supportsInterface(INTERFACE_ID_ERC721),
            "Not ERC721"
        );
        for(uint i=0; i < artion.length; i++) {
            if(artion[i] == _artion) {
                return true;
            }
        }
